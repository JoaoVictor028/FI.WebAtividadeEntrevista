$(document).ready(function () {
    ...
    $('#add-row').on('click', function () {
        ...
        if (nome && cpf) {
            if (validarCPFExistente(cpf) === false) {
                AddRowGrid(nome, cpf, undefined, $('#IdBeneficiario').val());
    ...
    $('#grid').on('click', '.atualizar-row', function () {
        ...
        $('#NomeBeneficiario').val($row.find('input[name*="Nome"]').val());
        $('#CPFBeneficiario').val(cpfb).val();
        $('#IdBeneficiario').val($row.find('input[name*="Id"]').val());
        $row.remove();
        reorganizarIndices();
    });
});